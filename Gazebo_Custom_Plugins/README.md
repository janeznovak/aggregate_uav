# Gazebo Custom Plugins

Plugin list:

- Moving actor with collision
  + GazeboActorRosPlanarMove
...
Add build directory to GAZEBO\_PLUGIN\_PATH

```bash
source install/local_setup.bash
```

Inside the sdf file add inside the actor tag:

```xml
<plugin name="actor1_plugin" filename="libgazebo_ros_actor_planar_move.so">
  ...
  <scaling collision="LowerBack_Spine_collision" scale="
    12.0 20.0 5.0 " pose="0.05 0 0 0 -0.2 0"/>
