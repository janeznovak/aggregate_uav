...
def abs2rel(x, y, z, angle, origin_x=0.0, origin_y=0.0, origin_z=0.0, rotation=0.0):
    """!
    ...
    if round(sin(rotation), 2) != 0.0:
        new_x = -new_x
        new_y = -new_y
    return round(new_x, 2), round(new_y, 2), round(new_z, 2), round(new_qz,2), round(new_qw, 2)
